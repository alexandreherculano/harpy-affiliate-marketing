...
class ResearchAgent:
    ...
    def discover_trends(self, niche: str, dry_run: bool = False) -> TrendResult:
        ...
        if dry_run:
            return self._mock_trends(niche)
    ...
    def find_affiliate_programs(self, query: str, dry_run: bool = False) -> ProgramResult:
        ...
        if dry_run:
            return self._mock_program(query)
    ...
    def analyze_traffic(self, product: str, dry_run: bool = False) -> TrafficResult:
        ...
        if dry_run:
            return self._mock_traffic(product)
    ...
    def run_full_research(
        ...
        logger.info("ResearchAgent starting for niche: %s (dry_run=%s)", niche, dry_run)

        trends = self.discover_trends(niche, dry_run=dry_run)

        program_query = niche
        if trends.top_format:
            program_query = f"{niche} {trends.top_format} tools"

        program = self.find_affiliate_programs(program_query, dry_run=dry_run)

        product_name = program.product_name or niche
        traffic = self.analyze_traffic(product_name, dry_run=dry_run)

        summary = self._build_summary(trends, program, traffic)
    ...
    def _mock_trends(self, niche: str) -> TrendResult:
        return TrendResult(
            niche=niche,
            top_format="comparison",
            avg_engagement=35.2,
            best_hook=f"I tested the top 3 {niche} tools so you don't have to",
            content_gap=f"No one is doing honest side-by-side {niche} comparisons on TikTok",
            benchmark_views=18000,
            raw="[DRY RUN] Mock trend data — no API call made.",
        )

    def _mock_program(self, query: str) -> ProgramResult:
        return ProgramResult(
            product_name=f"Top {query} Program",
            slug=query.lower().replace(" ", "-"),
            commission="30% recurring",
            cookie_days=60,
            stars=127,
            traffic_score=85,
            justification=f"Highest commission in the {query} space with recurring model and long cookie window.",
            raw="[DRY RUN] Mock program data — no API call made.",
        )

    def _mock_traffic(self, product: str) -> TrafficResult:
        return TrafficResult(
            product_name=product,
            monthly_visits="2.1M",
            global_rank="#45,230",
            bounce_rate="42%",
            top_sources=["Direct (35%)", "Organic Search (28%)", "Social (18%)"],
            score=82,
            verdict="Strong advertiser — healthy traffic mix and low bounce rate. Worth promoting.",
            raw="[DRY RUN] Mock traffic data — no API call made.",
        )
